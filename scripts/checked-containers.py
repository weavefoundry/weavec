...
def cache(args):
    ...
    with tempfile.TemporaryDirectory(prefix='weavec-container-cache-') as temporary:
        ...
        assert contents.startswith('weavec-summaries 19\n')
        metadata.write_text(contents.replace('weavec-summaries 19\n', 'weavec-summaries 18\n', 1))
        process, _ = link('sidecar-old-format')
        assert process.returncode == 1 and 'unsupported format 18' in process.stderr, process.stderr
