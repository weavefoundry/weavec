...
struct ExportedRequirement {
  ...
  /// The bytes one element of the pointee takes, so that a `counted` term
  /// becomes bytes at a caller that never saw the definition (§13.2 step
  /// 5); 0 when the pointee has no known size.
...
/// §13.1 `imports[].calls[].evidence[]`: what the caller knows about one
/// argument, so that the link step can decide the callee's inferred
/// requirements without the caller's AST (§13.2 step 5).
struct ArgumentEvidence {
  /// A pointer argument: the bytes it guarantees from where it points
  /// (§7.3's judgement, a lower bound). None when nothing is guaranteed.
  std::optional<std::uint64_t> bytes = std::nullopt;
  /// `bytes` is the object's whole size at offset 0, not a lower bound:
  /// the argument is a complete array, a string literal, or the address of
  /// a complete object. Only then is a longer requirement a violation.
  bool exact = false;
  /// The argument is the null pointer constant.
  bool null = false;
  /// An integer argument's value, when it is a constant expression.
  std::optional<std::int64_t> value = std::nullopt;

  friend bool operator==(const ArgumentEvidence &,
                         const ArgumentEvidence &) = default;
};

/// §13.1 `imports[].calls[]`: one call of the import. `args[i]` says
/// whether pointer argument `i` was Single-valid (§7.3); none for a
/// non-pointer argument. `evidence[i]` is what §13.2 step 5 decides the
/// callee's requirements with.
struct ImportCall {
