...
//
// Step 5 decides an exported requirement at a cross-unit caller from the
// caller's record (`verifyRequirements`), because kinds and must-access
// requirements are per-TU: a callee defined elsewhere has none in the
// caller's table, in its compile or in its re-run at link.
//
// What waits for engine facts: the §7.6 invariant verdicts (none are
// recorded, so A3 lists only conflicting slot kinds), and applying boundary
// rows to other units' facets (the engine would mark which facets rest on
// which entry assumption).
//
//===----------------------------------------------------------------------===//
...
/// §13.2 step 5: one inferred requirement (§7.5) of an exported function,
/// decided at one call in another member.
struct RequirementDecision {
  /// The calling member, the function in it and the Call site's ordinal.
  std::size_t member = 0;
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  std::string function = {};
  std::uint32_t site = 0;
  /// The callee and the parameter the requirement is about.
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  std::string callee = {};
  std::uint32_t param = 0;
  /// `proven`, `violation(out-of-bounds)` or `unresolved(unknown-extent)`.
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  core::FacetDecision decision = {};
  /// The bytes the callee needs and the bytes the argument gives, spelled
  /// for the ledger's `requirements` row; absent when not known.
  std::optional<std::string> need = std::nullopt;
  std::optional<std::string> have = std::nullopt;

  friend bool operator==(const RequirementDecision &,
                         const RequirementDecision &) = default;
};

struct RequirementCheck {
  /// One row per (requirement, cross-unit call), in member and call order.
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  std::vector<RequirementDecision> decisions = {};
  /// Requirements met at every cross-unit call the records contain: the
  /// A1 `verified` count.
  std::uint64_t verified = 0;
  /// Functions all of whose requirements are met at every call in the
  /// program, which is closed for them: `(member, name)`. Their
  /// `trusted(caller-contract)` rows become proven (§13.2 step 5).
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  std::set<std::pair<std::size_t, std::string>> discharged = {};
};

/// §13.2 step 5, first bullet: every cross-unit call to an exported
/// function with an inferred requirement, decided at the caller from what
/// its record says about the argument.
///
/// A violation is reported by step 4, not here: the caller's re-run sees
/// the callee's summary requirement and errors at the argument itself, with
/// more in view than a record holds. This adds the ledger's Call row, the
/// A1 counts and the discharge.
[[nodiscard]] RequirementCheck
verifyRequirements(std::span<const ProgramMember> members,
                   const LinkShape &shape);
...
struct ProgramLedgerInput {
  ...
  /// §13.2 step 5: what `verifyRequirements` decided, whose rows the
  /// program ledger carries and whose counts A1 reports.
  const RequirementCheck *requirements = nullptr;
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  LinkShape shape = {};
