...
struct PlannerOptions {
  ...
  /// §5.3: facets checked because a thread or signal handler shares their
  /// pointer; inexpressible ones are `trusted(concurrency)`.
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  std::set<std::pair<core::SiteId, core::Facet>> concurrencyDowngraded = {};
};
