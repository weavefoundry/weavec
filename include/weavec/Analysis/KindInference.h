...
struct UnitKinds {
  ...
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  KindTable table = {};
  /// §9.3's slots and their local solution (the engine consumes them from
  /// S7 on; `KindInference` judges indirect call results with them).
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
  ...
  [[nodiscard]] static std::shared_ptr<const UnitKinds>
  build(clang::ASTContext &context, const core::LibrarySpec &library);
};
