...
struct SiteInfo {
  ...
  /// Call sites: the arguments of a parameter whose Single default the
  /// callee relies on that are not Single-valid (the §7.3 Call-site row).
  // NOLINTNEXTLINE(readability-redundant-member-init): designated-init default
...
class SiteCollector {
public:
  /// `inferred`, when given, supplies the Single-valid judgements of the
  /// §7.3 Call-site rows.
  SiteCollector(clang::ASTContext &ctx, const KindTable &kindTable,
                const core::LibrarySpec &spec,
                const KindInferenceResult *kindInference = nullptr,
                const SlotCollection *unitSlots = nullptr,
                const core::SlotSolution *slotSolution = nullptr)
      : context(ctx), kinds(kindTable), library(spec), inferred(kindInference),
        slots(unitSlots), solution(slotSolution) {}
...
private:
  ...
  /// RFC 0030 §9.3: an indirect call whose slot is closed on one target a
  /// `LibrarySpec` row governs gets that row's sites, as a direct call to
  /// the target would.
