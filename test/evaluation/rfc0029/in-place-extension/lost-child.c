...
static int grow(struct node*n,unsigned depth){if(!depth)return 1;struct node*c=calloc(1,sizeof *c);if(!c)return 0;if(!grow(c,depth-1)){drop(c); return 0;} return 1;}
