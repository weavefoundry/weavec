...
int scan(struct reader *r) {
 ...
 for(i=0;r && r->position+i<r->capacity;i++) {
  switch((r->data+r->position)[i]) {
  case 'n':case 'a':case '0':case '1':++count;break;
...
int main(void){const unsigned char input[]="nan";struct reader r={input,0,sizeof input,0};return scan(&r);}
