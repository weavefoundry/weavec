...
struct pair make(int *p, int *q) {
  struct pair r = {*(int *)__weavec_chk_nonnull(p), ((int *)__weavec_chk_nonnull(q))[1]};
  return r;
}
int sum(int *p, int *q, int *r, int *t, int n) {
  int s = 0;
  for (int i = 0; i < n && *(int *)__weavec_chk_nonnull(p); i++)
    s += *(int *)__weavec_chk_nonnull(q);
  if (*(int *)__weavec_chk_nonnull(r) > 3)
    return s;
  return -*(int *)__weavec_chk_nonnull(t);
}
