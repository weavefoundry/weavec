...
// RUN: python3 -c "import json; d=json.load(open(r'%t.json')); assert d['version']==1; assert d['invocation_ok']; assert d['units'][0]['functions'][0]['selected']; assert d['units'][0]['functions'][0]['complete']"
