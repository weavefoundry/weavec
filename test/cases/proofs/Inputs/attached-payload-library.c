...
struct node *create(void){struct node *p=new_item(&global_hooks);if(p)p->flags=1;return p;}
void destroy(struct node *p){struct node *next=0;while(p){next=p->next;if(!(p->flags&256)&&p->child)destroy(p->child);if(!(p->flags&256)&&p->text){global_hooks.deallocate(p->text);p->text=0;}if(!(p->flags&512)&&p->key){global_hooks.deallocate(p->key);p->key=0;}global_hooks.deallocate(p);p=next;}}
...
int attach(struct node *object,struct node *item){char *key=0;if(!object||!item||object==item)return 0;key=(char*)malloc(4);if(!key)return 0;key[0]=0;item->key=key;return add(object,item);}
int attach_wrapper(struct node *o,struct node *i){return attach(o,i);}
void reset_hooks(void){reset(0);}
