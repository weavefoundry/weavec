...
| `recall/<CWE>/` | the recall pins (67 as `scripts/recall.py` counted them) |
...
scripts/run-cases.py --require checked --filter 'soundness/*_ok.c'   # gate G5
...
6. **Verify** (`--checks verify`). The build uses `-fweavec-checks=verify`
   ...
   whatever its markers. When both trap, `--lldb` asks `lldb` for the trap's
   category (`weavec.proven` or `weavec`); without it such a trap is taken as
   the unproven check's.
...
tool as `scripts/evaluate.py` ran it:
