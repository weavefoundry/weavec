// Root-cause repro realloc5: the append calls realloc itself; v0.10.0 reports nothing (a
// control case: the library's own outcome-keyed realloc summary).
// intended (RFC 0030 section 9.1, gate S7): no finding.
// CLEAN
// ASAN
#include <stdlib.h>
#include <string.h>
...
static void *my_realloc(void *ptr, size_t old, size_t n) {
    void *m;
    if (n == 0) { if (ptr) free(ptr); return NULL; }
    m = malloc(n);
