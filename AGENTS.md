...
9. Keep the engine seam and the library table clean (gate H2,
   ...
   `CallContextSummaries.cpp` and the `Dataflow*.cpp` files include
   `Dataflow.h`; `FunctionDataflow` publishes only through `LedgerAdapter`
...
| Change pointer kinds / how annotations become kinds | `lib/Core/PointerKind.cpp`, `lib/Analysis/AttributeReader.cpp`, `lib/Analysis/KindInference*.cpp` |
