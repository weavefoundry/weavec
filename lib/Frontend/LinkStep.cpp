...
core::Assumptions assumptionsOf(std::span<const ProgramMember> members,
  ...
  // Deciding a requirement at a caller needs the engine's facts there; until
  // it publishes them, none is verified.
  assumptions.a1.verified = 0;
...
core::Ledger composeProgramLedger(const ProgramLedgerInput &input) {
  ...
  ledger.assumptions = assumptionsOf(members, input.shape, relianceRows);
