...
llvm::json::Object toJson(const Payload &payload) {
  ...
  llvm::json::Array callbackGlobals;
  for (const auto &[global, targets] : exports.callbackGlobals) {
    llvm::json::Object entry;
    entry["global"] = utf8(global);
    entry["targets"] = targets.toString();
    callbackGlobals.push_back(std::move(entry));
  }
  llvm::json::Object contexts;
  contexts["memoryRequests"] = std::move(memoryRequests);
  contexts["callbackRequests"] = std::move(callbackRequests);
  contexts["callbackGlobals"] = std::move(callbackGlobals);
  json["contexts"] = std::move(contexts);
...
bool PayloadReader::readContexts(const llvm::json::Object &json) {
  ...
  const llvm::json::Array &globals = array(contexts, "callbackGlobals");
  for (std::size_t i = 0; i < globals.size(); ++i) {
    const std::string where =
        "payload.contexts.callbackGlobals[" + std::to_string(i) + "]";
    const llvm::json::Object &entry = object(globals[i]);
    const auto targets = core::CallTargets::parse(text(entry, "targets"));
    if (!targets || text(entry, "global").empty() ||
        !exports.callbackGlobals.emplace(text(entry, "global"), *targets)
             .second)
      return fail(where, "malformed or duplicate callback global");
  }
  return true;
}
