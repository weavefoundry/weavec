...
std::shared_ptr<const UnitKinds>
UnitKinds::build(clang::ASTContext &context, const core::LibrarySpec &library) {
  auto kinds = std::make_shared<UnitKinds>();
  kinds->table = AttributeReader(context, library).read();
  kinds->slots = SlotCollector(context, library).collect();
  kinds->solution = kinds->slots.solve();
  KindInferenceOptions options;
  options.slots = &kinds->slots;
  options.slotSolution = &kinds->solution;
  kinds->inferred =
      KindInference(context, library, options).infer(kinds->table);
  return kinds;
}
