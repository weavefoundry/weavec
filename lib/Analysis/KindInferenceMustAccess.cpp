...
static bool testsNullness(const clang::Expr *condition,
  ...
  return (variableOf(binary->getLHS()) == &param && isNull(binary->getRHS())) ||
         (variableOf(binary->getRHS()) == &param && isNull(binary->getLHS()));
}
...
static std::optional<CountedLoop> countedLoop(const clang::ForStmt &loop,
  ...
  if (index == nullptr || index->hasGlobalStorage() ||
    ...
    return std::nullopt;
  // An index whose address is taken may change through a pointer.
  if (const auto use = c.state.variableUses.find(index);
      use != c.state.variableUses.end() && use->second.addressTaken)
    return std::nullopt;
...
/// The bytes from the local's value that `access` needs, when constant:
/// the object width of the local's pointee for `*b` and `b->f` (R1), up to
/// the element read for a subscript of a trailing member array
/// (`b->y[10]`), and `(k + 1)` elements for `b[k]` (R5).
static std::optional<std::int64_t> copyAccessBytes(const Access &access,
                                                   const MustAccessContext &c) {
  const clang::ASTContext &context = c.state.context;
  const clang::QualType pointee = access.base->getType()->getPointeeType();
  const auto width = static_cast<std::int64_t>(objectWidth(pointee, context));
  if (width <= 0)
    return std::nullopt;
  if (access.form == Access::Form::Index) {
    const auto k = c.state.constantOf(access.index);
    const auto elements =
        k && *k >= 0 ? llvm::checkedAdd(*k, std::int64_t{1}) : std::nullopt;
    const auto size = context.getTypeSizeInChars(pointee).getQuantity();
    return elements ? llvm::checkedMul(*elements, size) : std::nullopt;
  }
  if (access.form != Access::Form::Element)
    return std::nullopt;
  const auto *member = llvm::dyn_cast<clang::MemberExpr>(access.expr);
  const auto *field =
      member != nullptr
          ? llvm::dyn_cast<clang::FieldDecl>(member->getMemberDecl())
          : nullptr;
  const clang::ConstantArrayType *array =
      field != nullptr ? context.getAsConstantArrayType(field->getType())
                       : nullptr;
  if (array == nullptr || field->isBitField())
    return width;
  // `b->y[k]`: the subscript's element is inside the object only if the
  // member bounds it; a trailing one may reach past the object width.
  const clang::Stmt *user = c.parents.getParentIgnoreParenCasts(member);
  const auto *subscript =
      llvm::dyn_cast_or_null<clang::ArraySubscriptExpr>(user);
  const auto k = subscript != nullptr ? c.state.constantOf(subscript->getIdx())
                                      : std::nullopt;
  if (!k || *k < 0)
    return width;
  const auto offset = static_cast<std::int64_t>(context.getFieldOffset(field) /
                                                context.getCharWidth());
  const auto element =
      context.getTypeSizeInChars(array->getElementType()).getQuantity();
  const auto count = llvm::checkedAdd(*k, std::int64_t{1});
  const auto bytes = count ? llvm::checkedMul(*count, element) : std::nullopt;
  const auto end = bytes ? llvm::checkedAdd(offset, *bytes) : std::nullopt;
  if (!end)
    return std::nullopt;
  return std::max(width, *end);
}
...
requirementOf(const Access &access, const CountedLoop *loop,
  ...
  case Access::Form::Element:
    // `p->y[k]` of a trailing (flexible) member may reach past the object
    // width: the requirement is the bytes it reaches (§7.4, §7.5 R1).
    if (const auto bytes = copyAccessBytes(access, c);
        bytes &&
        std::cmp_greater(*bytes,
                         objectWidth(access.base->getType()->getPointeeType(),
                                     c.state.context))) {
      out.kind = inferred(core::PointerKind::sized(
          core::ExtentTerm::constant(*bytes), core::Nullability::Nonnull));
      out.rule = MustAccessRule::R1;
      // The access it covers is the subscript.
      out.access = c.parents.getParentIgnoreParenCasts(access.expr);
      return out;
    }
...
// -- Local copies of a parameter
// ---------------------------------------------------

/// §7.5: the locals initialised from a parameter of `params`, through
/// conversions between object pointer types (`struct big *b = v;`), when
/// neither the local nor the parameter is ever modified or has its address
/// taken: an access through the local is one through the parameter.
static llvm::DenseMap<const clang::VarDecl *, const clang::ParmVarDecl *>
localCopies(const clang::Stmt *stmt,
            const llvm::DenseSet<const clang::VarDecl *> &params,
            const KindInferenceState &state) {
  llvm::DenseMap<const clang::VarDecl *, const clang::ParmVarDecl *> out;
  const auto untouched = [&](const clang::VarDecl *variable) {
    const auto use = state.variableUses.find(variable);
    return use == state.variableUses.end() ||
           (!use->second.assigned && !use->second.modified &&
            !use->second.addressTaken);
  };
  const std::function<void(const clang::Stmt *)> walk =
      [&](const clang::Stmt *each) {
        if (each == nullptr)
          return;
        if (const auto *decl = llvm::dyn_cast<clang::DeclStmt>(each))
          for (const clang::Decl *d : decl->decls()) {
            const auto *local = llvm::dyn_cast<clang::VarDecl>(d);
            if (local == nullptr || !local->isLocalVarDecl() ||
                local->isStaticLocal() || !local->hasInit() ||
                !isObjectPointer(local->getType()) || !untouched(local))
              continue;
            const clang::ParmVarDecl *param =
                parameterOf(stripPointerCasts(local->getInit()));
            if (param != nullptr && params.contains(param) && untouched(param))
              out.try_emplace(local, param);
          }
        for (const clang::Stmt *child : each->children())
          walk(child);
      };
  walk(stmt);
  return out;
...
void KindInferenceState::inferMustAccess() {
  ...
  for (const clang::FunctionDecl *definition : definitions) {
    ...
    llvm::DenseMap<const clang::ParmVarDecl *,
        ...
        found;
    // One requirement per kind and guard; the accesses that give it are
    // the ones it covers.
    const auto add = [&](const clang::ParmVarDecl *param,
      ...
      const auto same =
          llvm::find_if(list, [&](const MustAccessRequirement &each) {
            return each.kind == requirement.kind &&
                   each.guard == requirement.guard;
          });
      if (same != list.end()) {
        if (!llvm::is_contained(same->covers, requirement.access))
          same->covers.push_back(requirement.access);
        return;
      }
      requirement.covers = {requirement.access};
    ...
    // R1 and R5 through a local copy of a parameter, in bytes: the copy may
    // view the object as another type (§7.5; probe 09's `struct big *b =
    // v; b->y[10]`). Only where the direct calls enforce the requirement:
    // an exported function's (a `qsort` comparator's) stays unresolved.
    const auto owner = functions.find(definition->getCanonicalDecl());
    const bool enforcedAtCalls =
        !definition->isExternallyVisible() &&
        (owner == functions.end() || !owner->second.addressTaken);
    const auto copies = enforcedAtCalls
                            ? localCopies(definition->getBody(), params, *this)
                            : decltype(localCopies(nullptr, params, *this)){};
    if (!copies.empty()) {
      std::vector<Access> through;
      collectAccesses(
          definition->getBody(), c,
          [&](const clang::VarDecl *variable) {
            return copies.contains(variable);
          },
          through);
      for (const Access &access : through) {
        const clang::ParmVarDecl *param = copies.lookup(access.base);
        const auto at = position(access.expr);
        if (!at || !mustAccessAlways(c, *param, at->first, at->second))
          continue;
        const bool tested = llvm::any_of(
            blocksBefore(at->first, {}, nullptr),
            [&](const clang::CFGBlock *before) {
              return testsNullness(llvm::dyn_cast_or_null<clang::Expr>(
                                       before->getTerminatorCondition()),
                                   *access.base, context);
            });
        const auto bytes = tested ? std::nullopt : copyAccessBytes(access, c);
        if (!bytes || *bytes <= 0)
          continue;
        add(param, MustAccessRequirement{.kind = core::PointerKind::sized(
                                             core::ExtentTerm::constant(*bytes),
                                             core::Nullability::Nonnull,
                                             core::KindSource::Inferred),
                                         .guard = std::nullopt,
                                         .rule = MustAccessRule::R1,
                                         .access = access.expr});
      }
    }
