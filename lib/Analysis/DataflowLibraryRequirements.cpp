...
void FunctionDataflow::decideArgumentRequirements(
  ...
  const auto decideAgainst = [&](unsigned argument, const Target &target,
    ...
    const auto checkedOr = [&](core::UnresolvedReason otherwise) {
      if (!core::isCheckOperand(target.known.extentClass)) {
        publish(argument,
      ...
      auto witness = lengthWitness(target, std::move(needTerm));
      publish(argument,
              witness
  ...
  for (const ArgumentRequirement &requirement : requirements) {
    ...
    if (requirement.libraryObject) {
      ...
      decideAgainst(i, *target, need, requirement.needTerm);
    }
  }
}
