...
llvm::ArrayRef<CoveringRequirement>
KindTable::covering(const clang::Stmt &access) const {
  if (!coveringBuilt) {
    coveringIndex.clear();
    for (const auto &[key, entry] : params)
      for (unsigned r = 0; r < entry.mustAccess.size(); ++r) {
        const MustAccessRequirement &requirement = entry.mustAccess[r];
        const CoveringRequirement covered{
            .function = key.first, .param = key.second, .requirement = r};
        if (requirement.covers.empty() && requirement.access != nullptr)
          coveringIndex[requirement.access].push_back(covered);
        for (const clang::Stmt *stmt : requirement.covers)
          coveringIndex[stmt].push_back(covered);
      }
    coveringBuilt = true;
  }
  const auto found = coveringIndex.find(&access);
