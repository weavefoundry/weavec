...
Build and run your test suite with `weavec-cc`. Unproven null and bounds obligations are checked by default, and a failing check stops the program at the bad access. To collect every failing check in one run, build the tests with `-fweavec-checks=report`, which prints `weavec: runtime check failed: <kind> at <file>:<line>:<column>` and continues.
