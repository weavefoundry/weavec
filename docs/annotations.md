...
| `lifetime-too-short`  | error / warning | A pointer may outlive what it points to: `'<p>' may outlive '<x>', which it points to` (stored into an outer scope, a global or through a parameter) or `returned pointer may outlive '<x>', which it points to`. Notes: where `<x>` is declared and where it goes out of scope. Reported at the store, but decided when `<x>` dies ([RFC 0011](rfcs/0011-spatial-safety.md)): a store undone before then (`ls->fs = fs.prev`), or into a holder that is dead by then, is not reported. |
...
whole-program step); `weavec-cc --help-weavec` lists them. RFC 0030 removes
`-fweavec-strict` (`--strict-externs`), `-fweavec-report-unannotated`,
`-fweavec-analyze-headers` and `-fweavec-exclusive-borrows`
(`--exclusive-borrows`).
