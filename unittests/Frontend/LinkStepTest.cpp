...
TEST(LinkStep, ExportedRequirementsAreDecidedAtTheCallersInOtherUnits) {
  // §13.2 step 5: `fill` requires `counted(n)` of `p` when `0 < n`. Three
  // calls in another unit: 12 bytes for 3 ints (met), 8 bytes for 5 ints
  ...
  define(callee, "fill", core::FunctionSummary{},
         record::FunctionInterface{
             ...
             .requirements = {record::ExportedRequirement{
                 ...
                 .element = 4}},
             .location = at("fill.c", 1, 6)});
  ProgramMember caller = member("main.c");
  caller.payload.exports.imports = {"fill"};
  const auto call = [](std::uint32_t site, std::optional<std::uint64_t> bytes,
                       bool exact, std::int64_t count) {
    return record::ImportCall{
        .function = "main",
        .site = site,
        .args = {true, std::nullopt},
        .evidence = {record::ArgumentEvidence{
                         .bytes = bytes, .exact = exact, .null = false},
                     record::ArgumentEvidence{.value = count}}};
  };
  caller.payload.facts.imports["fill"] = record::ImportInterface{
      .declared = {.params = {record::DeclaredParam{.name = "p"},
                              record::DeclaredParam{.name = "n"}}},
      .location = at("main.c", 1, 6),
      .calls = {call(0, 12, true, 3), call(1, 8, true, 5),
                call(2, std::nullopt, false, 3)}};
  const std::vector<ProgramMember> members{caller, callee};
  LinkShape shape;
  const RequirementCheck check = verifyRequirements(members, shape);
  ASSERT_EQ(check.decisions.size(), 3U);
  EXPECT_EQ(check.decisions[0].decision.compact(), "proven");
  EXPECT_EQ(check.decisions[0].need, std::optional<std::string>("12 bytes"));
  EXPECT_EQ(check.decisions[1].decision.compact(), "violation");
  EXPECT_EQ(check.decisions[1].decision.detail,
            "'fill' accesses 20 bytes of the argument for 'p', which is 8 "
            "bytes long");
  EXPECT_EQ(check.decisions[2].decision.compact(), "unresolved/unknown-extent");
  // Not every call meets it, so nothing is verified or discharged.
  EXPECT_EQ(check.verified, 0U);
  EXPECT_TRUE(check.discharged.empty());

  // A guard that does not hold accesses nothing, so `fill(NULL, 0)` meets
  // the requirement; with only met calls, A1 counts it verified and the
  // definition's `trusted(caller-contract)` rows become proven.
  caller.payload.facts.imports["fill"].calls = {
      call(0, 12, true, 3),
      record::ImportCall{
          .function = "main",
          .site = 1,
          .args = {true, std::nullopt},
          .evidence = {record::ArgumentEvidence{.bytes = 0, .null = true},
                       record::ArgumentEvidence{.value = 0}}}};
  const std::vector<ProgramMember> met{caller, callee};
  const RequirementCheck all = verifyRequirements(met, shape);
  ASSERT_EQ(all.decisions.size(), 2U);
  EXPECT_EQ(all.decisions[1].decision.compact(), "proven");
  EXPECT_EQ(all.verified, 1U);
  EXPECT_EQ(all.discharged.size(), 1U);

  core::Ledger callerRun = runOf({ledgerFunction(
      "main",
      {SiteSpec{core::SiteKind::Call, "fill", FacetDecision::proven()},
       SiteSpec{core::SiteKind::Call, "fill", FacetDecision::proven()}},
      core::Linkage::External)});
  core::Ledger calleeRun =
      runOf({ledgerFunction("fill", {SiteSpec{core::SiteKind::Index, "", {}}},
                            core::Linkage::External)});
  calleeRun.units.front()
      .functions.front()
      .sites.front()
      .addFacet(core::Facet::Spatial)
      .decide(FacetDecision::trustedFor(core::TrustReason::CallerContract,
                                        "'fill' requires counted(n) of 'p'"));
  const std::vector<const core::Ledger *> runs{&callerRun, &calleeRun};
  const core::Ledger ledger =
      composeProgramLedger(ProgramLedgerInput{.members = met,
                                              .runs = runs,
                                              ...
                                              .linkDiagnostics = {},
                                              .cwd = "/work"});
  const core::FacetRecord *atCall =
      siteOf(ledger, 0, "main", 0).facet(core::Facet::Spatial);
  ASSERT_NE(atCall, nullptr);
  ASSERT_EQ(atCall->requirements.size(), 1U);
  EXPECT_EQ(atCall->requirements.front().argument,
            std::optional<std::uint32_t>(0));
  EXPECT_EQ(atCall->requirements.front().have,
            std::optional<std::string>("12 bytes"));
  const core::FacetRecord *inBody =
      siteOf(ledger, 1, "fill", 0).facet(core::Facet::Spatial);
  ASSERT_NE(inBody, nullptr);
  EXPECT_EQ(inBody->decision.compact(), "proven");
  EXPECT_EQ(inBody->decision.detail,
            "every caller of 'fill' in the program meets the requirement");
  ASSERT_TRUE(ledger.assumptions);
  EXPECT_EQ(ledger.assumptions->a1.exportedRequirements, 1U);
  EXPECT_EQ(ledger.assumptions->a1.verified, 1U);
  EXPECT_EQ(ledger.assumptions->unverifiedRequirements(), 0U);
}
