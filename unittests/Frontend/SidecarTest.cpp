...
TEST(Sidecar, RejectsOtherFormatsAndMalformedLines) {
  ...
  EXPECT_FALSE(parseUnitRecord("weavec-summaries 21\nfunction f\n", &error));
  ...
  EXPECT_FALSE(
      parseUnitRecord("weavec-summaries 21\nreported x y z\n", &error));
...
TEST(Sidecar, MalformedTypedCountsAreRejected) {
  for (const auto *line :
       {"a b 0", "a b -4", "a b 4 i64", "a b 4 u0", "a b 4 u65", "a b 4 b1",
        "a b 256 u8", "a b 4 u64 extra"}) {
    std::string error;
    EXPECT_FALSE(parseUnitRecord(
        std::string("weavec-summaries 21\nsized-field ") + line + "\n", &error))
        << line;
    EXPECT_FALSE(error.empty());
  }
}
