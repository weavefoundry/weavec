...
void use(struct s *o, int *q) {
  ...
  const SiteIndex sites =
      SiteCollector(after->context(), after->kinds,
                    core::LibrarySpec::shipped(), &after->inferred)
          .collect();
  ...
  for (const char *function : {"first", "cursor"}) {
    EXPECT_FALSE(describe(sites, function).empty()) << function;
    EXPECT_EQ(describe(sites, function), describe(before.sites, function))
        << function;
  }
  // §7.5: the static `first`'s requirement (`p[0]`, R1) gives its direct
  // call spatial and null facets; no required position appears.
  std::vector<std::string> use = describe(before.sites, "use");
  ASSERT_EQ(use.size(), 4U);
  EXPECT_EQ(use[2], "call/call first(q+2) temporal");
  use[2] = "call/call first(q+2) spatial null temporal";
  EXPECT_EQ(describe(sites, "use"), use);
}
