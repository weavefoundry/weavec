...
void fine(char *p) { p[0] = 1; free(p); }
  ...
  EXPECT_EQ(outcomes(piped.ledger, "g")[1],
            "p[0] spatial=proven null=checked temporal=violation");
  EXPECT_EQ(outcomes(piped.ledger, "h")[1], "p[0] spatial=proven null=checked "
  ...
  EXPECT_EQ(outcomes(piped.ledger, "fine"),
            (Lines{"p[0] spatial=proven null=checked temporal=proven",
