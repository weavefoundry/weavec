...
TEST(FunctionSummary, BorrowKindKeepsRootAndSubtreeBoundaries) {
  const auto p = SummaryPath::param(1);
  FunctionSummary summary;
  summary.addEffect(SummaryPath::param(0).deref(), {.written = true});
  summary.addEffect(p, {.read = true});
  summary.addEffect(p.field("by_value"), {.written = true});
  summary.addEffect(p.indexed("0"), {.written = true});
  summary.addEffect(SummaryPath::param(2).deref(), {.written = true});
  summary.addEffect(SummaryPath::global(1).deref(), {.written = true});
  EXPECT_EQ(summary.borrowKind(1), std::nullopt);
  summary.addEffect(p.deref().field("nested").indexed("0").deref(),
                    {.read = true});
  EXPECT_EQ(summary.borrowKind(1), BorrowKind::Shared);
  summary.addStore(
      {.dest = p.deref().field("output"), .value = ValueSource::null()});
  EXPECT_EQ(summary.borrowKind(1), BorrowKind::Mutable);
  summary.stores.clear();
  summary.addEffect(p.deref().field("nested").indexed("1").deref(),
                    {.moved = true});
  EXPECT_EQ(summary.borrowKind(1), BorrowKind::Mutable);
}
